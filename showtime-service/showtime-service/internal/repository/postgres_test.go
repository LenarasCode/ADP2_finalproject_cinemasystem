...
func TestGetShowtime_Integration(t *testing.T) {
	...
	_, err := repo.db.Exec(`INSERT INTO showtimes (id, movie_id, hall, start_time, available_seats) VALUES ('a1', 'm1', 'H1', '2025-01-01T10:00:00Z', 80)`)
	if err != nil {
		t.Fatal(err)
	}

	s, err := repo.GetShowtime("a1")
	if err != nil {
		t.Fatal(err)
	}
	if s.ID != "a1" || s.AvailableSeats != 80 {
